...
integration-dependencies: kyma ## create k3d cluster and run integration test
	# connectivity proxy
	...
	kubectl wait --for condition=Available -n docker-registry deployment dockerregistry-operator --timeout=90s
	sleep 5
	kubectl wait --for condition=Available -n docker-registry deployment dockerregistry --timeout=60s
	sleep 5
