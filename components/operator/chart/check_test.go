...
func TestCheckCRDOrphanResources(t *testing.T) {
	...
	_ = cache.Set(context.Background(), noCRDManifestKey,
		RegistryProxySpecManifest{Manifest: fmt.Sprint(testDeploy)})
	_ = cache.Set(context.Background(), noOrphanManifestKey,
		RegistryProxySpecManifest{Manifest: fmt.Sprint(testCRD, separator, testDeploy)})
	_ = cache.Set(context.Background(), oneOrphanManifestKey,
		RegistryProxySpecManifest{Manifest: fmt.Sprint(testCRD, separator, testOrphanCR)})
	_ = cache.Set(context.Background(), emptyManifestKey,
		RegistryProxySpecManifest{Manifest: ""})
	_ = cache.Set(context.Background(), wrongManifestKey,
		RegistryProxySpecManifest{Manifest: "api: test\n\tversion: test"})
